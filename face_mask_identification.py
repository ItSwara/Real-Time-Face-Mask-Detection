...
model = tf.keras.models.load_model("final_inception_face_mask_detection.h5")
...
while True:
    ...
    # Pass gray image to Classifier for identification of face,# it returns the co-ordinates of face area in terms of x, y, w, h
    ...
    if images_faces_count <= face_count_frame:
        for count in range(images_faces_count):
            ...
            if face_lable == "mask":
                cv2.putText(frame, "Wearing Mask", (startX, startY), cv2.FONT_HERSHEY_SIMPLEX,1.2,(0,220,0),2)
                cv2.rectangle(frame,
                              (startX, startY), (endX, endY),
                              (0, 220, 0), 3)
            else:
                cv2.putText(frame, "No Mask", (startX, startY),  cv2.FONT_HERSHEY_SIMPLEX, 1,(0, 0, 255), 2)
                cv2.rectangle(frame,
                              (startX, startY), (endX, endY),
                              (0, 0, 220), 3)
